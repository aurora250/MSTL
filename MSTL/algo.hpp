...
template <typename Iterator1, typename Iterator2, typename Iterator3, enable_if_t<
	is_fwd_iter_v<Iterator1> && is_fwd_iter_v<Iterator2> && is_fwd_iter_v<Iterator3>, int> = 0>
...
template <typename Iterator, typename BinaryPredicate, enable_if_t<
...
Iterator adjacent_find(Iterator first, Iterator last) {
	return MSTL::adjacent_find(first, last, MSTL::equal_to<iter_val_t<Iterator>>());
}

template <typename Iterator, typename T, typename BinaryPredicate, enable_if_t<
	is_input_iter_v<Iterator>, int> = 0>
...
iter_dif_t<Iterator> count(Iterator first, Iterator last, const T& value) {
	return MSTL::count_if(first, last, value, MSTL::equal_to<iter_val_t<Iterator>>());
}
...
Iterator find(Iterator first, Iterator last, const T& value) {
	return MSTL::find_if(first, last, value);
}
...
Iterator1 search(Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2) {
	return MSTL::search(first1, last1, first2, last2, MSTL::equal_to<iter_val_t<Iterator1>>());
}
...
Iterator1 find_first_of(Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2) {
	return MSTL::find_first_of(first1, last1, first2, last2, MSTL::equal_to<iter_val_t<Iterator1>>());
}

template <typename Iterator, typename Function, enable_if_t<
	is_input_iter_v<Iterator>, int> = 0>
...
template <typename Iterator, typename Generator, enable_if_t<
	is_input_iter_v<Iterator>, int> = 0>
...
bool includes(Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2) {
	return MSTL::includes(first1, last1, first2, last2, MSTL::less<iter_val_t<Iterator1>>());
}

template <typename Iterator1, typename Iterator2, typename Iterator3, typename Compare, enable_if_t<
	is_fwd_iter_v<Iterator1> && is_fwd_iter_v<Iterator2> && is_fwd_iter_v<Iterator3>, int> = 0>
...
Iterator3 merge(Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2, Iterator3 result) {
	return MSTL::merge(first1, last1, first2, last2, result, MSTL::less<iter_val_t<Iterator1>>());
}
...
template <typename Iterator, typename T>
inline Iterator lomuto_partition(Iterator first, Iterator last, const T& pivot) {
	return MSTL::lomuto_partition(first, last, pivot);
}

template <typename Iterator1, typename Iterator2, typename T, enable_if_t<
	is_fwd_iter_v<Iterator1> && is_fwd_iter_v<Iterator2>, int> = 0>
...
template <typename Iterator1, typename Iterator2, typename Predicate, enable_if_t<
	is_fwd_iter_v<Iterator1> && is_fwd_iter_v<Iterator2>, int> = 0>
...
template <typename Iterator1, typename Iterator2, typename Predicate, typename T, enable_if_t<
	is_fwd_iter_v<Iterator1> && is_fwd_iter_v<Iterator2>, int> = 0>
Iterator2 replace_copy_if(Iterator1 first, Iterator1 last, Iterator2 result, 
...
template <typename Iterator, typename Predicate, typename T, enable_if_t<
	is_fwd_iter_v<Iterator>, int> = 0>
...
template <typename Iterator, typename Distance, enable_if_t<
	is_rnd_iter_v<Iterator>, int> = 0>
void __rotate_cycle_aux(Iterator first, Iterator last, Iterator initial, Distance shift) {
...
void rotate(Iterator first, Iterator middle, Iterator last) {
	...
	while (n--)
		MSTL::__rotate_cycle_aux(first, last, first + n, middle - first);
}
...
template <typename Iterator1, typename Iterator2, typename UnaryOperation, enable_if_t<
	is_fwd_iter_v<Iterator1> && is_fwd_iter_v<Iterator2>, int> = 0>
...
template <typename Iterator1, typename Iterator2, typename Iterator3, typename BinaryOperation, enable_if_t<
	is_fwd_iter_v<Iterator1> && is_fwd_iter_v<Iterator2> && is_fwd_iter_v<Iterator3>, int> = 0>
...
Iterator2 unique_copy(Iterator1 first, Iterator1 last, Iterator2 result) {
	return MSTL::unique_copy(first, last, result, MSTL::equal_to<iter_val_t<Iterator1>>());
}
...
Iterator lower_bound(Iterator first, Iterator last, const T& value) {
	return MSTL::lower_bound(first, last, value, MSTL::less<iter_val_t<Iterator>>());
}
...
Iterator upper_bound(Iterator first, Iterator last, const T& value) {
	return MSTL::upper_bound(first, last, value, MSTL::greater<iter_val_t<Iterator>>());
}
...
template <typename Iterator1, typename Iterator2, typename BinaryPred, enable_if_t<
...
bool is_permutation(Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2) {
	return MSTL::is_permutation(first1, last1, first2, last2, MSTL::equal_to<iter_val_t<Iterator1>>());
}
...
bool next_permutation(Iterator first, Iterator last) {
	return MSTL::next_permutation(first, last, MSTL::less<iter_val_t<Iterator>>());
}
...
bool prev_permutation(Iterator first, Iterator last) {
	return MSTL::prev_permutation(first, last, MSTL::less<iter_val_t<Iterator>>());
}
...
pair<Iterator, Iterator> equal_range(Iterator first, Iterator last, const T& value) {
	return MSTL::equal_range(first, last, value, MSTL::less<iter_val_t<Iterator>>());
}

template <typename Iterator, typename Distance, typename Compare>
void __merge_without_buffer_aux(Iterator first, Iterator middle, Iterator last,
	...
	MSTL::__merge_without_buffer_aux(first, first_cut, new_middle, len11, len22, comp);
	MSTL::__merge_without_buffer_aux(new_middle, second_cut, last, len1 - len11, len2 - len22, comp);
}

template <typename Iterator1, typename Iterator2, typename Distance>
Iterator1 __rotate_with_buffer_aux(Iterator1 first, Iterator1 middle, Iterator1 last,
...
void __merge_with_buffer_aux(Iterator first, Iterator middle, Iterator last,
	...
	if (len1 <= len2 && len1 <= buffer_size) {
		...
		Iterator new_middle = MSTL::__rotate_with_buffer_aux(
			first_cut, middle, second_cut, len1 - len11, len22, buffer, buffer_size);

		MSTL::__merge_with_buffer_aux(
...
void inplace_merge(Iterator first, Iterator middle, Iterator last, Compare comp) {
	...
	if (buffer.begin() == 0)
		MSTL::__merge_without_buffer_aux(first, middle, last, len1, len2, comp);
	else
		MSTL::__merge_with_buffer_aux(
...
void inplace_merge(Iterator first, Iterator middle, Iterator last) {
	return MSTL::inplace_merge(first, middle, last, MSTL::less<iter_val_t<Iterator>>());
}
