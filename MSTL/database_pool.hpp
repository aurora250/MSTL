...
struct database_settings final {
    // default: root
    static _MSTL string& username() {
        static _MSTL string username = "root";
        return username;
    }
    static void username(const _MSTL string& username) {
        database_settings::username() = username;
    }

    // set it as you wish
    static _MSTL string& password() {
        static _MSTL string password{};
        return password;
    }
    static void password(const _MSTL string& password) {
        database_settings::password() = password;
    }

    // set it as you wish
    static _MSTL string& dbname() {
        static _MSTL string dbname{};
        return dbname;
    }
    static void dbname(const _MSTL string& dbname) {
        database_settings::dbname() = dbname;
    }

    // default: 127.0.0.1
    static _MSTL string& ip() {
        static _MSTL string ip = "127.0.0.1";
        return ip;
    }
    static void ip(const _MSTL string& ip) {
        database_settings::ip() = ip;
    }

    // default: 3306
    static _MSTL uint16_t& port() {
        static _MSTL uint16_t port = 3306;
        return port;
    }
    static void port(const uint16_t port) {
        database_settings::port() = port;
    }

    // default: utf8mb4
    static _MSTL string& character_set() {
        static _MSTL string character_set = "utf8mb4";
        return character_set;
    }
    static void character_set(const _MSTL string& character_set) {
        database_settings::character_set() = character_set;
    }
};
...
public:
    ...
    explicit database_result(MYSQL_RES* result) noexcept
...
public:
    ...
    MSTL_NODISCARD bool connect_to(
        ...
        const uint32_t port, const _MSTL string& character_set) const noexcept {
    ...
    MSTL_NODISCARD database_result query(const _MSTL string& sql) const noexcept {
        ...
        return database_result{mysql_store_result(mysql)};
    }
