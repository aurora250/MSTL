...
struct connect_config {
    ...
    static connect_config for_mysql(const string& db) {
    ...
    static connect_config for_sqlite(const string& file) {
        connect_config config;
...
struct i_database_result {
    using size_type         = size_t;
    using difference_type   = ptrdiff_t;

    virtual ~i_database_result() = default;
...
struct i_database_connect {
    using clock_type = std::clock_t;

    virtual ~i_database_connect() = default;
    ...
    virtual bool connect_to(const _MSTL string& user, const _MSTL string& password,
        ...
        uint32_t port, const _MSTL string& character_set) const = 0;
    virtual bool connect_to(const connect_config& config) const = 0;
    ...
    virtual unique_ptr<i_database_result> query(const string& sql) const = 0;
    virtual bool connected() const = 0;
    virtual bool is_valid() const = 0;
    virtual void close() const = 0;
    virtual void refresh_alive() = 0;
    virtual clock_type get_alive() const = 0;
    virtual bool reset_connect(const connect_config& config) = 0;
};

class i_database_factory {
protected:
    connect_config config_;

public:
    explicit i_database_factory(const connect_config& config)
       : config_(config) {}

    virtual ~i_database_factory() = default;
    virtual i_database_connect* create_connection() = 0;
    virtual i_database_result* create_result(void* native_result) = 0;
};


struct db_mysql_result final : i_database_result {
private:
...
struct db_mysql_connect final : i_database_connect {
private:
    MYSQL* mysql = nullptr;
...
public:
    ...
    MSTL_NODISCARD bool connect_to(
        ...
        const _MSTL string& dbname, const _MSTL string& ip,
        const uint32_t port, const _MSTL string& character_set) const noexcept override {
    ...
    MSTL_NODISCARD bool connect_to(const connect_config& config) const noexcept override {
        return connect_to(
            config.username,
            config.password,
            config.database,
    ...
    MSTL_NODISCARD unique_ptr<i_database_result> query(const _MSTL string& sql) const noexcept override {
        if (mysql_query(mysql, sql.c_str())) {
            return {};
        }
    ...
    MSTL_NODISCARD bool reset_connect(const connect_config& config) override {
        if (connected()) {
            mysql_close(mysql);
            mysql = mysql_init(nullptr);
...
class db_mysql_factory final : public i_database_factory {
public:
    explicit db_mysql_factory(const connect_config& config)
        : i_database_factory(config) {}

    i_database_connect* create_connection() override {
    ...
    i_database_result* create_result(void* native_result) override {
...
#ifdef MSTL_SUPPORT_SQLITE3__
struct db_sqlite_result final : i_database_result {
private:
...
struct db_sqlite_connect final : i_database_connect {
private:
...
public:
    ...
    bool connect_to(const _MSTL string&, const _MSTL string&,
            ...
            uint32_t, const _MSTL string&) const override {
        sqlite3_open(dbname.c_str(), &db);
        return connect_to_file(dbname);
    }

    bool connect_to(const connect_config& config) const override {
    ...
    MSTL_NODISCARD unique_ptr<i_database_result> query(const string& sql) const override {
        if (!connected()) return {};
...
private:
    bool connect_to_file(const string& file_path) const {
        if (connected()) {
            close();
        }
...
class db_sqlite_factory final : public i_database_factory {
public:
    explicit db_sqlite_factory(const connect_config& config)
        : i_database_factory(config) {}
...
private:
    ...
    unique_ptr<i_database_factory> factory_ = nullptr;
    _MSTL queue<i_database_connect*> connect_queue_;
    ...
    void produce_connect_task() {
        while (true) {
            ...
            if (connect_queue_.size() < max_size_) {
                auto* p = factory_->create_connection();
                if (p != nullptr) {
                    p->refresh_alive();
    ...
    void scanner_connect_task() {
        while (true) {
            ...
            while (connect_queue_.size() > init_size_) {
                i_database_connect* ptr = connect_queue_.front();
...
public:
    database_pool(
        const DB_TYPE type, const connect_config& config,
    ...
    _MSTL shared_ptr<i_database_connect> get_connect() {
        std::unique_lock<std::mutex> lock(queue_mtx_);
        ...
        while (connect_queue_.empty()) {
            if (cv_.wait_for(lock,
                ...
                if (connect_queue_.empty()) {
                    if (connect_queue_.size() < max_size_) {
                        auto* new_conn = factory_->create_connection();
                        if (new_conn != nullptr) {
        ...
        i_database_connect* raw_conn = connect_queue_.front();
        connect_queue_.pop();
        ...
        if (!raw_conn->is_valid()) {
            try {
                if (!raw_conn->reset_connect(config_)) {
                    delete raw_conn;
                    raw_conn = factory_->create_connection();
                    if (raw_conn == nullptr) {
                        cv_.notify_all();
                        return nullptr;
                    }
        ...
        auto conn_ptr = _MSTL shared_ptr<i_database_connect>(raw_conn,
            [this](i_database_connect* p) {
