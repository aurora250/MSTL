...
// object is not function, reference and void type. 
...
// don't contain non-static members, virtual functions and virtual base class. 
// base class should also be an empty type
template <class T>
...
// standard layout type have these characteristics:
// consistent access control for non-static members.
// no virtual functions and virtual base classes.
// no base class or there is only one base class and the base class is a standard layout type.
...
// trivial type have these characteristics:
// the default constructor, copy constructor, move constructor, copy assignment operator, 
...
// we can use memory comparison to determine equality.
// standard layout types and trivial types have unique object representations.
template <class T>
...
#endif
// the following types are aggregate type:
// array type;
// class type(include struct and union) that have following characteristics:
...
struct make_signed {
    static_assert(is_nonbool_integral<T> || is_enum_v<T>,
        "make signed only support non bool integral and enum types");
...
// simulate the decline of function parameter:
//   ref and cv qualifier will be removed;
//   array will be converted to pointer;
//   function will be converted to function pointer.
template <class T>
struct decay {
...
// ADL (Argument Dependent Lookup) is a lookup mechanism.
// when a function is called, in addition to the global and local scopes, 
