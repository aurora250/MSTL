...
constexpr T accumulate(Iterator first, Iterator second, T init, BinaryOperation binary_op) {
	for (; first != second; ++first) init = binary_op(init, *first);
	return init;
}

template <typename Iterator, typename T>
constexpr T accumulate(Iterator first, Iterator second, T init) {
...
constexpr Iterator2 adjacent_difference(Iterator1 first, Iterator1 last,
...
constexpr Iterator2 adjacent_difference(Iterator1 first, Iterator1 last, Iterator2 result) {
...
template <typename Iterator1, typename Iterator2, typename BinaryOperation, enable_if_t<
	is_input_iter_v<Iterator1> && is_input_iter_v<Iterator2>, int> = 0>
constexpr Iterator2 partial_sum(
...
template <typename Iterator1, typename Iterator2>
constexpr Iterator2 partial_sum(Iterator1 first, Iterator1 last, Iterator2 result) {
